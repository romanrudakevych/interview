...
const rawQuestions = [
  ...
  // ----------------------------------------------------------------- Git --
  {
    ...
    shortAnswer:
      "Git — это система контроля версий, которая позволяет отслеживать изменения в коде, возвращаться к старым версиям и работать над проектом совместно. GitHub — это платформа, где можно хранить код в облаке, делиться им и управлять проектами с помощью Git.",
    longAnswer:
      "Git — это инструмент, который следит за всеми изменениями в вашем проекте. Например, вы можете вернуться к старой версии кода, если допустили ошибку, или параллельно разрабатывать новые функции без риска сломать основную версию. Git работает локально на вашем компьютере и ведёт историю изменений прямо на вашем устройстве.\n\nGitHub — это веб-сервис, который использует Git для хранения проектов в облаке. Он добавляет удобные функции для командной работы: pull request'ы, code review, issues, CI/CD, вики и управление доступом. Существуют и аналоги — GitLab, Bitbucket. Git можно использовать и без GitHub, но вместе они позволяют эффективно работать над проектом в команде.",
  ...
  {
    question: "Что такое локальный и удалённый репозитории в Git?",
    shortAnswer:
      "Локальный репозиторий находится на вашем компьютере и хранит всю историю изменений проекта. Удалённый репозиторий — это копия проекта, размещённая в облаке (например, на GitHub). Вы работаете с локальным репозиторием и периодически синхронизируете изменения с удалённым.",
    longAnswer:
      "Локальный репозиторий — это папка на вашем компьютере, где Git хранит весь код проекта и историю изменений. Вы можете работать с этим репозиторием даже без подключения к интернету: вносить изменения, создавать ветки и фиксировать изменения (commit).\n\nУдалённый репозиторий находится в облаке, например на GitHub или GitLab. Он нужен, чтобы делиться проектом с другими разработчиками или работать над ним с разных устройств.\n\nПример работы: вы создаёте локальный репозиторий командой `git init`, добавляете удалённый репозиторий командой `git remote add origin <URL>` и отправляете изменения в удалённый репозиторий командой `git push`. Это разделение позволяет безопасно работать с кодом локально и синхронизировать изменения, когда нужно.",
    codeExample:
      "git init\ngit remote add origin <URL>\ngit push -u origin main",
    skills: ["Git"],
    keywords: ["#repository", "#remote"],
    difficulty: 2,
    rating: 3,
  },
  {
    question: "Опишите шаги по выгрузке новых изменений в удалённый репозиторий Git",
    shortAnswer:
      "Чтобы выгрузить изменения, выполните три шага: добавьте файлы в индекс (`git add <file>` или `git add .`), зафиксируйте изменения (`git commit -m \"Описание\"`) и отправьте их в удалённый репозиторий (`git push`).",
    longAnswer:
      "Когда вы работаете с проектом, процесс выгрузки изменений обычно состоит из трёх шагов.\n\n1. Добавление файлов в индекс. Команда `git add` подготавливает файлы к фиксации. Например, `git add index.html` для одного файла или `git add .` для всех изменённых.\n\n2. Фиксация изменений. Командой `git commit` вы сохраняете изменения в локальной истории репозитория. Сообщение коммита должно быть осмысленным: `git commit -m \"Добавил секцию о преимуществах продукта\"`.\n\n3. Отправка изменений в удалённый репозиторий. Команда `git push` выгружает ваши изменения в удалённое хранилище. Если вы работаете в основной ветке: `git push origin main`.\n\nЭти шаги повторяются каждый раз, когда нужно синхронизировать локальные изменения с удалённым репозиторием.",
    codeExample:
      "git add .\ngit commit -m \"Описание изменений\"\ngit push origin main",
    skills: ["Git"],
    keywords: ["#add", "#commit", "#push"],
    difficulty: 3,
    rating: 4,
  },
  {
    ...
    shortAnswer:
      "Команда `git status` показывает текущее состояние репозитория: какие файлы были изменены, добавлены в индекс или остаются неотслеживаемыми. Это помогает понять, какие шаги нужно выполнить дальше.",
    longAnswer:
      "Команда `git status` — это ваш помощник, который сообщает, что происходит в репозитории. Она показывает:\n\n- Изменённые файлы, которые не добавлены в индекс. Git напомнит, что их нужно добавить.\n- Файлы, добавленные в индекс, но ещё не зафиксированные. Вы подготовили изменения для коммита, но не завершили процесс.\n- Файлы, которые не отслеживаются Git. Это новые файлы, которые ещё не добавлены в репозиторий.\n\nЭта информация помогает понять, что нужно сделать дальше: добавить файлы, зафиксировать изменения или проигнорировать ненужные файлы.",
    codeExample:
  ...
  },
  {
    ...
    longAnswer:
      "Git управляет процессом фиксации изменений через промежуточный этап — индекс (staging area). Команда `git add` переносит изменения из рабочей области в этот индекс.\n\nНапример, вы изменили файл `app.js`. Чтобы подготовить этот файл к фиксации, выполните `git add app.js`. Если нужно добавить сразу все изменённые файлы: `git add .`.\n\nТеперь `git status` покажет, что файл находится в индексе и готов к фиксации. Однако команда сама по себе не фиксирует изменения, она лишь отмечает их для будущего коммита.",
    codeExample: "git add app.js\ngit add .",
    skills: ["Git"],
    keywords: ["#add", "#staging-area"],
    difficulty: 2,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Команда `git commit` сохраняет изменения из индекса в историю репозитория. Она фиксирует текущий прогресс, позволяя вернуться к этой точке в будущем. Каждый коммит сопровождается описанием.",
    longAnswer:
      "`git commit` — это шаг, который фиксирует ваши изменения в истории проекта. Команда берёт все изменения, добавленные в индекс (`git add`), и сохраняет их в виде «снимка».\n\nПример использования: `git commit -m \"Добавил функцию поиска\"`. Сообщение должно быть коротким, но информативным, чтобы в будущем было понятно, что именно изменилось. Например, вместо «Исправил баг» лучше написать «Исправил баг с некорректным отображением списка товаров».\n\nПосле выполнения команды изменения сохраняются в истории, но остаются только в локальном репозитории. Чтобы отправить их в удалённое хранилище, нужно использовать `git push`. Каждый коммит — это шаг, к которому можно вернуться, если возникнут проблемы.",
    codeExample: 'git commit -m "Добавил функцию поиска"',
    skills: ["Git"],
    keywords: ["#commit"],
    difficulty: 3,
    rating: 4,
  ...
  {
    question: "Что делают команды git push и git pull?",
    shortAnswer:
      "Команда `git push` отправляет ваши локальные изменения в удалённый репозиторий. Команда `git pull` скачивает изменения из удалённого репозитория и объединяет их с вашей локальной версией.",
    longAnswer:
      "После фиксации изменений командой `git commit` команда `git push` отправляет эти изменения в удалённый репозиторий, например на GitHub: `git push origin main`. Здесь `origin` — имя удалённого репозитория, а `main` — ветка, куда отправляются изменения.\n\nКогда кто-то вносит изменения в удалённый репозиторий, вы используете `git pull`, чтобы скачать их в свой локальный репозиторий и объединить с вашей текущей веткой: `git pull origin main`. Это автоматически выполняет две команды: `git fetch` (скачивание изменений) и `git merge` (объединение изменений с вашей веткой).",
  ...
  },
  {
    ...
    shortAnswer:
      "`git fetch` только скачивает изменения из удалённого репозитория, не изменяя локальные файлы. `git pull` скачивает эти изменения и сразу объединяет их с вашей текущей веткой.",
    longAnswer:
      "`git fetch` скачивает все новые данные из удалённого репозитория — изменения в ветках, новые коммиты. Однако эти изменения сохраняются только в удалённой копии ветки (`origin/main`) и не затрагивают вашу локальную ветку. После этого вы можете вручную просмотреть и объединить изменения командой `git merge origin/main` или сначала изучить их через `git log origin/main`.\n\n`git pull` выполняет два действия сразу: скачивает изменения (`git fetch`) и автоматически объединяет их с текущей веткой (`git merge`). Таким образом, `git pull` удобен для быстрого обновления, а `git fetch` полезен, если вы хотите сначала изучить изменения перед объединением. С `git pull` всё объединяется автоматически, что может вызвать конфликты, если ваши изменения пересекаются с чужими.",
  ...
  {
    question: "Как посмотреть, что именно вы изменили в определённом файле Git?",
    shortAnswer:
      "Используйте команду `git diff <имя_файла>` для просмотра изменений в конкретном файле в рабочей области. Для уже добавленных в индекс изменений используйте `git diff --staged`.",
    longAnswer:
      "Команда `git diff` показывает разницу между текущей версией файла в рабочей области и последним зафиксированным состоянием.\n\nЕсли вы изменили файл `app.js`, но ещё не добавили его в индекс, выполните `git diff app.js`. Вы увидите строки с изменениями: удалённые строки помечаются знаком `-`, добавленные — знаком `+`.\n\nЕсли вы уже добавили изменения в индекс (`git add`), используйте `git diff --staged`.\n\nЭта команда помогает понять, что вы изменили, и убедиться, что всё сделано правильно, перед созданием коммита.",
    codeExample:
  ...
  {
    question: "Почему мы делаем git pull, а затем git push?",
    shortAnswer:
      "Мы делаем `git pull`, чтобы сначала синхронизировать локальный репозиторий с последними изменениями из удалённого. Затем выполняем `git push`, чтобы отправить свои изменения. Это помогает избежать конфликтов и сохранить согласованность данных между участниками команды.",
    longAnswer:
      "Когда вы работаете в команде, другие разработчики могут вносить изменения в удалённый репозиторий. Если вы попытаетесь выполнить `git push`, не имея последних изменений, Git заблокирует операцию, чтобы предотвратить перезапись чужих изменений.\n\nПроцесс:\n\n1. `git pull` — скачивает и объединяет последние изменения из удалённого репозитория, обновляя ваш локальный код.\n2. Работа над проектом — вы вносите изменения, добавляете их в индекс (`git add`) и фиксируете (`git commit`).\n3. `git push` — после синхронизации и внесения изменений вы отправляете свой код в удалённый репозиторий.\n\nПример: ваш коллега добавил новый файл `feature.js` и отправил его. Вы делаете `git pull`, чтобы получить этот файл, вносите свои изменения и выполняете `git push`, чтобы обновить репозиторий без конфликтов.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#pull", "#push", "#workflow"],
    difficulty: 3,
    rating: 4,
  },
  {
    question: "Как вы разрешаете конфликты в Git?",
    shortAnswer:
      "Конфликты возникают, когда изменения из удалённого и локального репозиториев пересекаются. Чтобы разрешить конфликт, нужно вручную отредактировать проблемные файлы, выбрать правильную версию, а затем зафиксировать изменения.",
    longAnswer:
      "Конфликты возникают, когда Git не может автоматически объединить изменения. Пример: вы и ваш коллега изменили одну и ту же строку в файле.\n\nШаги для разрешения конфликта:\n\n1. Выполните `git pull`. Если возникает конфликт, Git остановится и покажет, в каких файлах проблема.\n2. Откройте конфликтующие файлы. Вы увидите метки, которые указывают конфликтующие части: `<<<<<<< HEAD`, `=======`, `>>>>>>> origin/main`.\n3. Отредактируйте файл, удаляя метки и выбирая правильную версию.\n4. Сохраните изменения и добавьте файл в индекс: `git add <имя_файла>`.\n5. Завершите процесс с помощью `git commit`.\n\nСовет: используйте инструменты для разрешения конфликтов, например встроенные редакторы в VS Code или специализированные программы (Sourcetree, GitKraken).",
    codeExample:
  ...
  },
  {
    ...
    longAnswer:
      "Ветка — это своего рода «копия» проекта на определённый момент времени, где вы можете вносить изменения, не влияя на основную версию кода. Например, основная ветка часто называется `main`, и от неё можно создавать другие ветки для разработки новых функций.\n\nКоманды для работы с ветками:\n\n- Создать ветку: `git branch feature/new-feature`\n- Переключиться на ветку: `git checkout feature/new-feature`\n- Создать и сразу переключиться: `git checkout -b feature/new-feature`\n\nПреимущества ветвления: параллельная работа (каждый разработчик работает в своей ветке), безопасность (изменения не затрагивают основную ветку) и история изменений. После завершения работы ветки объединяют с основной командой `git merge`.",
    codeExample:
  ...
  },
  {
    ...
    shortAnswer:
      "Основные стратегии: централизованный рабочий процесс (Centralized Workflow), рабочий процесс разветвления функций (Feature Branching Workflow), Gitflow и Forking Workflow.",
    longAnswer:
      "Централизованный рабочий процесс (Centralized Workflow). Веткой разработки по умолчанию является `main`, и все изменения фиксируются в этой ветке. Этот процесс не требует других веток, кроме главной.\n\nРабочий процесс разветвления функций (Feature Branching Workflow). Основная идея в том, что разработка всех функций должна вестись в специальной ветке, а не в основной.\n\nРабочий процесс Gitflow. Определяет строгую модель ветвления, разработанную вокруг релиза проекта. Назначает специфические роли различным веткам и определяет, как и когда они должны взаимодействовать. В дополнение к функциональным веткам используются отдельные ветки для подготовки, поддержки и записи релизов.\n\nРабочий процесс Forking. Принципиально отличается тем, что вместо единого серверного репозитория каждый разработчик получает свой серверный репозиторий. То есть у каждого участника два репозитория: частный локальный и публичный серверный. Чаще всего Forking Workflow встречается в публичных проектах с открытым исходным кодом.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#branching-strategy", "#workflow"],
    difficulty: 5,
    rating: 4,
  },
  {
    ...
    longAnswer:
      "Когда вы работаете над проектом, в нём могут быть файлы, которые не нужно добавлять в репозиторий, например: временные файлы (`.tmp`, `.log`), сгенерированные папки (`node_modules`, `dist`), конфиденциальные данные (ключи API, пароли).\n\nВ файл `.gitignore` записываются шаблоны для таких файлов и папок.\n\nGit будет игнорировать указанные файлы, но только если они ещё не добавлены в репозиторий. Если файл уже был зафиксирован, его нужно удалить из индекса: `git rm --cached <имя_файла>`.\n\n`.gitignore` упрощает работу и предотвращает случайное добавление ненужных файлов в репозиторий.",
    codeExample:
  ...
  },
  {
    ...
    longAnswer:
      "Рабочий процесс Gitflow структурирует работу с ветками, чтобы сделать её упорядоченной и понятной. Основные компоненты:\n\n- Ветка `main`: содержит стабильный код, готовый для релиза.\n- Ветка `develop`: используется для активной разработки, все новые функции добавляются сюда.\n- Feature-ветки: создаются из `develop` для работы над новыми функциями, после завершения сливаются обратно в `develop`.\n- Release-ветки: создаются для подготовки релиза, в них фиксируют баги и добавляют документацию, после завершения сливаются в `main` и `develop`.\n- Hotfix-ветки: создаются из `main` для быстрого исправления ошибок в продакшене, после исправления сливаются в `main` и `develop`.\n\nПреимущества: разделение разработки и релизов, удобное управление функциями, багами и релизами. Gitflow хорошо подходит для сложных проектов, но может быть избыточным для небольших команд.",
    codeExample:
      "git checkout -b feature/awesome-feature develop\ngit checkout -b hotfix/critical-bug main",
    skills: ["Git"],
    keywords: ["#gitflow", "#branching-strategy"],
    difficulty: 4,
    rating: 4,
  },
  {
    ...
    longAnswer:
      "Git предоставляет множество преимуществ:\n\n- Отслеживание изменений: видно, кто и когда внёс изменения, можно сравнивать версии и возвращаться к предыдущим состояниям проекта.\n- Распределённость: каждый разработчик имеет полную копию репозитория, что обеспечивает надёжность — данные не теряются даже при сбоях на сервере.\n- Ветвление и слияние: можно создавать независимые ветки для новых функций или исправлений, а затем сливать их с основной веткой.\n- Работа в команде: Git синхронизирует изменения между участниками проекта, предотвращая конфликты и перезапись данных.\n- Быстродействие: локальные операции происходят очень быстро, потому что Git работает локально.\n- Интеграция с инструментами: Git легко интегрируется с платформами вроде GitHub, GitLab, Bitbucket.\n\nGit помогает разработчикам эффективно управлять кодом, ускоряет командную работу и делает процесс разработки более надёжным.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#git", "#advantages"],
    difficulty: 2,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Распределённая система контроля версий (например, Git) сохраняет полную копию репозитория у каждого участника проекта. Это делает работу более надёжной, позволяет работать офлайн и обеспечивает лёгкое слияние изменений.",
    longAnswer:
      "В распределённых системах контроля версий (Distributed Version Control Systems, DVCS) каждый разработчик имеет локальную копию полного репозитория, включая всю историю изменений. Примеры таких систем: Git, Mercurial.\n\nПреимущества распределённых систем:\n\n- Независимость от центрального сервера: все операции (фиксация изменений, сравнение версий) выполняются локально, а сервер используется только для синхронизации.\n- Работа офлайн: можно работать над проектом без подключения к интернету.\n- Резервное копирование: если сервер выходит из строя, репозиторий можно восстановить с любого локального компьютера.\n- Упрощённая работа с ветками: локальные ветки легко создавать и объединять без влияния на других разработчиков.\n\nОтличие от централизованных систем (например, Subversion): там все данные хранятся на одном сервере, что делает их уязвимыми к сбоям и требует постоянного подключения.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#dvcs", "#version-control"],
    difficulty: 3,
    rating: 3,
  },
  {
  ...
  },
  {
    ...
    longAnswer:
      "Когда вы используете `git clone`, Git копирует весь репозиторий на ваш локальный компьютер. Это включает все файлы проекта в их текущем состоянии и историю изменений (все коммиты, ветки и теги).\n\nПример использования: `git clone https://github.com/username/project.git`.\n\nПосле выполнения команды Git создаёт папку `project` (или заданное имя), загружает в неё содержимое репозитория и настраивает связь с удалённым репозиторием (`origin`).\n\nДополнительные параметры позволяют настроить ветку, глубину копирования или имя папки: `git clone -b develop --depth 1 https://github.com/username/project.git custom-folder`.",
    codeExample:
  ...
  },
  {
    ...
    longAnswer:
      "Иногда вам нужно временно отложить текущую работу, чтобы переключиться на другую задачу, но изменения ещё не готовы для коммита. В таких случаях помогает `git stash`.\n\nОсновные действия:\n\n- `git stash` — сохранение изменений (удаляет изменения из рабочей директории и индекса, сохраняя их в стек).\n- `git stash list` — просмотр сохранённых изменений.\n- `git stash apply` — применение изменений, оставляя их в стеке.\n- `git stash pop` — применение и удаление из стека.\n- `git stash drop` — удаление из стека.\n\nПример: вы работаете в ветке `main` и хотите переключиться на ветку `feature`, но не хотите терять свои текущие изменения — `git stash`, `git checkout feature`, затем `git stash apply`.",
    codeExample:
  ...
  },
  {
    ...
    shortAnswer:
      "Для удаления локальной ветки: `git branch -d <branch_name>` (или `git branch -D <branch_name>` при несмёрженных изменениях). Для удаления удалённой ветки: `git push origin --delete <branch_name>`.",
    longAnswer:
      "Удаление веток помогает поддерживать порядок в проекте, особенно после завершения работы над задачей.\n\nУдаление локальной ветки:\n\n- `git branch -d <branch_name>` — удаление ветки, если её изменения уже слиты.\n- `git branch -D <branch_name>` — принудительное удаление, если ветка не слита.\n\nУдаление удалённой ветки: `git push origin --delete <branch_name>`.\n\nПример полного процесса: вы закончили работу над `feature`-веткой, слили её в `develop` и хотите удалить — `git checkout develop`, `git branch -d feature`, `git push origin --delete feature`.\n\nВажно убедиться, что работа в ветке завершена, прежде чем её удалять.",
    codeExample:
  ...
  },
  {
    ...
    shortAnswer:
      "`git reflog` показывает историю всех перемещений указателя HEAD, включая коммиты, слияния и переключения веток. Эта команда полезна для восстановления потерянных коммитов или веток.",
    longAnswer:
      "Git хранит журнал всех изменений HEAD — это и есть reflog. Даже если вы удалили ветку или коммит, он остаётся в reflog в течение 30 дней (по умолчанию).\n\nКоманда `git reflog` показывает список всех операций, включая их хэши, например:\n\na1b2c3d HEAD@{0}: commit: Updated README\nd4e5f6g HEAD@{1}: checkout: moving from feature to main\n\nПрименение reflog:\n\n- Восстановление потерянного коммита: если вы случайно удалили ветку или сбросили коммит — `git reset --hard <commit_hash>`.\n- Просмотр операций в репозитории: видно, какие действия были выполнены, даже если их нет в `git log`.\n\nПример: если вы сбросили ветку `feature`, но хотите её восстановить — `git reflog`, затем `git checkout -b feature <commit_hash>`.\n\n`git reflog` — это спасательный инструмент для работы с историей изменений.",
    codeExample:
      "git reflog\ngit reset --hard <commit_hash>\ngit checkout -b feature <commit_hash>",
    skills: ["Git"],
    keywords: ["#reflog", "#recovery"],
    difficulty: 4,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Для отмены коммита, который был отправлен на сервер, используйте команду `git revert`, которая создаёт новый коммит, отменяющий изменения. Это безопасный метод, так как сохраняется история изменений.",
    longAnswer:
      "Когда коммит уже был отправлен на удалённый репозиторий (например, командой `git push`), его нельзя удалить без риска повредить историю, особенно если другие разработчики уже могли его скачать. Вместо этого используется команда `git revert`, которая создаёт новый коммит, отменяющий изменения.\n\nИспользование `git revert`: команда `git revert <commit_hash>` создаёт новый коммит, который инвертирует изменения, сделанные в указанном коммите. Пример: `git revert abc1234`, затем `git push origin main`. Это безопасно, потому что отменяется только результат коммита, а сама история сохраняется.\n\nИспользование `git reset` для локальных изменений (не рекомендуется для публичных коммитов): если коммит ещё не был отправлен на сервер, можно использовать `git reset`. Но если коммит уже отправлен и опубликован, лучше использовать `git revert`, чтобы избежать конфликтов.\n\nУдаление коммитов (`git reset` или `git push --force`) на уже обнародованных данных может создать проблемы для других разработчиков из-за несогласованности истории.",
    codeExample: "git revert abc1234\ngit push origin main",
    skills: ["Git"],
  ...
  },
  {
    ...
    shortAnswer:
      "Если ветка была удалена локально, её можно восстановить, используя команду `git reflog`. Если ветка была удалена на сервере, можно восстановить её локально и запушить обратно командой `git push origin <branch_name>`.",
    longAnswer:
      "Восстановление локальной ветки. Если ветка была удалена локально, вы можете восстановить её с помощью `git reflog`, который хранит историю всех перемещений HEAD. Нужно найти коммит, на котором ветка была активной, и создать новую ветку: `git reflog`, затем `git checkout -b <branch_name> <commit_hash>`.\n\nВосстановление удалённой ветки (если она была удалена с сервера). Если ветка была удалена из удалённого репозитория, её можно восстановить, запушив её обратно на сервер: `git checkout -b <branch_name> <commit_hash>`, затем `git push origin <branch_name>`.\n\nПримечание: если вы работаете в команде, всегда проверяйте, кто ещё может зависеть от этой ветки, перед восстановлением.",
    codeExample:
      "git reflog\ngit checkout -b <branch_name> <commit_hash>\ngit push origin <branch_name>",
    skills: ["Git"],
    keywords: ["#branch", "#recovery", "#reflog"],
    difficulty: 4,
    rating: 3,
  },
  {
    question: "Как завершить слияние на стадии конфликта?",
    shortAnswer:
      "При возникновении конфликта в Git нужно вручную разрешить расхождения в файлах, добавить исправленные файлы в индекс, а затем завершить слияние коммитом (или командой `git merge --continue`).",
    longAnswer:
      "Конфликты слияния возникают, когда Git не может автоматически объединить изменения в двух ветках, потому что изменения в одном и том же участке кода несовместимы. Git помечает такие участки в файлах специальными метками, и вам нужно вручную выбрать, какие изменения оставить.\n\nШаги для разрешения конфликта:\n\n1. Переключитесь на ветку, в которую хотите влить изменения: `git checkout main`.\n2. Запустите команду слияния: `git merge feature-branch`.\n3. Разрешите конфликты вручную. Откройте файлы с метками `<<<<<<< HEAD`, `=======`, `>>>>>>> feature-branch`, уберите метки и выберите нужные изменения.\n4. Добавьте исправленные файлы в индекс: `git add <file_name>`.\n5. Завершите слияние: `git merge --continue` (или `git commit`).\n6. Отправьте изменения в удалённый репозиторий: `git push origin main`.\n\nКонфликты — нормальная часть работы с Git, и правильное их разрешение позволяет сохранить целостность проекта.",
    codeExample:
      "git checkout main\ngit merge feature-branch\n# разрешить конфликты\ngit add <file_name>\ngit merge --continue\ngit push origin main",
    skills: ["Git"],
    keywords: ["#merge-conflict", "#merge"],
    difficulty: 4,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Команда `git cherry-pick` позволяет выбрать и применить отдельный коммит из другой ветки в вашу текущую ветку, минуя другие изменения. Это полезно, когда нужно перенести только один коммит, а не весь набор изменений.",
    longAnswer:
      "`git cherry-pick` позволяет извлечь и применить отдельные коммиты из других веток. Это полезно, если вы хотите перенести конкретные изменения из одной ветки в другую, не сливая всю ветку. Например, вы хотите перенести исправление ошибки из ветки `feature-branch` в основную ветку `main`.\n\nПример использования:\n\n1. Переключитесь на ветку, в которую хотите перенести коммит: `git checkout main`.\n2. Используйте `git cherry-pick` с хешем коммита, который хотите перенести: `git cherry-pick abc1234`.\n\nGit применяет изменения из коммита `abc1234` к вашей текущей ветке и создаёт новый коммит. Если возникает конфликт при выполнении `cherry-pick`, вам нужно будет разрешить его вручную и завершить процесс слияния, как и в случае с обычным слиянием.",
  ...
  {
    question: "Как найти коммит, который что-то нарушил после операции слияния?",
    shortAnswer:
      "Для поиска коммита, который нарушил код, можно использовать команду `git bisect`. Этот инструмент помогает найти проблемный коммит путём бинарного поиска по истории коммитов.",
    longAnswer:
      "После слияния с веткой могут возникнуть проблемы, и нужно понять, какой коммит вызвал ошибку. Для этого помогает инструмент `git bisect`, который выполняет бинарный поиск по истории коммитов, постепенно сокращая диапазон возможных нарушителей.\n\nШаги для использования `git bisect`:\n\n1. Инициализируйте бинарный поиск: `git bisect start`.\n2. Укажите хороший коммит (где всё работало) и плохой коммит (где возникла ошибка): `git bisect good <good_commit_hash>`, `git bisect bad <bad_commit_hash>`.\n3. Git будет поочерёдно переключаться на коммиты между хорошим и плохим состоянием. После каждого переключения вам нужно проверить, возникает ли ошибка (`git bisect good` / `git bisect bad`).\n4. После нахождения проблемного коммита: `git bisect reset`.\n\nGit автоматически выполнит поиск и сообщит, какой коммит вызвал проблему.",
    codeExample:
      "git bisect start\ngit bisect good abc1234\ngit bisect bad def5678\n# ... проверки ...\ngit bisect reset",
    skills: ["Git"],
    keywords: ["#bisect", "#debugging"],
    difficulty: 5,
    rating: 3,
  },
  {
    question: "Что делают команды git reset --mixed и git merge --abort?",
    shortAnswer:
      "`git reset --mixed` отменяет изменения, добавленные в индекс, но сохраняет изменения в рабочем каталоге. `git merge --abort` отменяет слияние, которое находится в процессе, и возвращает репозиторий в состояние до начала слияния.",
    longAnswer:
      "`git reset --mixed`. Эта команда отменяет изменения, которые были добавлены в индекс с помощью `git add`, но оставляет изменения в рабочем каталоге (внесённые вами изменения в файлах сохраняются). Это полезно, когда вы хотите отменить добавление файлов в индекс, но не потерять сами изменения. `--mixed` — режим по умолчанию для `git reset`.\n\n`git merge --abort`. Когда вы выполняете слияние и возникает конфликт, вы можете использовать `git merge --abort`, чтобы отменить процесс слияния и вернуться в состояние до его начала. Это полезно, если вы не хотите разрешать конфликты и хотите остановить слияние. Команда возвращает репозиторий в состояние до начала слияния, включая отмену всех изменений в рабочей директории, связанных с этим слиянием.\n\nТаким образом, `git reset --mixed` полезен для работы с индексом, а `git merge --abort` — для прерывания слияния.",
    codeExample: "git reset --mixed\ngit merge --abort",
    skills: ["Git"],
  ...
  },
  {
    ...
    shortAnswer:
      "Для объединения последних N коммитов в один используется команда `git rebase -i HEAD~N`, которая позволяет интерактивно редактировать коммиты. В процессе вы выбираете, какие коммиты объединить, с помощью команды `squash` или `fixup`.",
    longAnswer:
      "Объединение нескольких коммитов в один называется сжатием (squashing) коммитов и выполняется с помощью интерактивного ребейза.\n\n1. Откройте интерактивный ребейз: `git rebase -i HEAD~N`, где N — количество последних коммитов, которые вы хотите объединить. Например, чтобы объединить последние 3 коммита, используйте `HEAD~3`.\n2. В редакторе вы увидите список коммитов (pick a1b2c3d Commit 1, pick d4e5f6g Commit 2, pick h7i8j9k Commit 3).\n3. Измените `pick` на `squash` или `fixup` для всех коммитов, кроме первого.\n4. Сохраните и закройте редактор. Git объединит коммиты в один.\n5. При необходимости отредактируйте сообщение для нового объединённого коммита.\n6. Завершите ребейз: `git rebase --continue`.\n\nЭтот процесс улучшает читаемость истории.",
    codeExample:
      "git rebase -i HEAD~3\n# pick -> squash для всех, кроме первого\ngit rebase --continue",
    skills: ["Git"],
    keywords: ["#rebase", "#squash"],
    difficulty: 5,
    rating: 4,
  },
  {
    ...
    longAnswer:
      "Git-хуки — это скрипты, которые могут быть автоматически вызваны в ответ на определённые действия в репозитории. Хуки находятся в папке `.git/hooks` вашего репозитория. Они могут быть написаны на различных языках (например, Bash или Python), но обычно используются для автоматических проверок, например линтинга, тестирования или блокировки нежелательных операций.\n\nПримеры хуков:\n\n- `pre-commit` — выполняется перед каждым коммитом. Может использоваться для проверки кода на ошибки или форматирования.\n- `post-commit` — выполняется после выполнения коммита, может использоваться для уведомлений или обновлений.\n- `pre-push` — выполняется перед пушем в удалённый репозиторий, используется для проверки изменений перед отправкой.\n- `commit-msg` — выполняется перед записью сообщения коммита, обычно используется для проверки формата сообщения.\n\nЧтобы включить хук, достаточно сделать его исполняемым: `chmod +x .git/hooks/pre-commit`.",
    codeExample: "chmod +x .git/hooks/pre-commit",
    skills: ["Git"],
    keywords: ["#hooks", "#pre-commit"],
    difficulty: 4,
    rating: 3,
  },
  {
    question: "Чем отличается git clone от git remote?",
    shortAnswer:
      "`git clone` используется для создания копии удалённого репозитория на вашем локальном компьютере, включая все его коммиты и историю. `git remote` управляет удалёнными репозиториями, позволяя добавлять, удалять или изменять удалённые URL-адреса для вашего локального репозитория.",
    longAnswer:
      "`git clone`. Команда используется для копирования удалённого репозитория на ваш локальный компьютер. При этом копируются все файлы, коммиты и история репозитория. Обычно эта команда используется один раз для создания локальной копии удалённого репозитория. Пример: `git clone https://github.com/user/repository.git`.\n\n`git remote`. Команда используется для управления удалёнными репозиториями. Она позволяет добавлять новые удалённые репозитории, изменять их URL или удалять их. Например, чтобы добавить удалённый репозиторий: `git remote add origin https://github.com/user/repository.git`. Команда `git remote -v` позволяет просматривать список всех удалённых репозиториев, связанных с локальным.\n\nОсновное отличие: `git clone` создаёт локальную копию репозитория, а `git remote` управляет удалёнными репозиториями, связанными с локальным.",
    codeExample:
      "git clone https://github.com/user/repository.git\ngit remote add origin https://github.com/user/repository.git\ngit remote -v",
    skills: ["Git"],
    keywords: ["#clone", "#remote"],
    difficulty: 3,
    rating: 3,
  },
  {
  ...
  },
  {
    ...
    longAnswer:
      "Команда `git revert` используется для отмены изменений, сделанных в одном или нескольких коммитах, путём создания нового коммита, который фактически отменяет изменения предыдущего.\n\nВ отличие от команды `git reset`, которая изменяет историю репозитория, `git revert` сохраняет историю и создаёт новый коммит, который «откатывает» изменения, добавленные в определённом коммите.\n\nПример использования: чтобы отменить изменения в конкретном коммите, используйте его хеш — `git revert <commit-hash>`. После этого Git создаст новый коммит, который отменит изменения, внесённые в указанном коммите.\n\nЭто полезно, если вы хотите отменить коммит, который уже был отправлен в удалённый репозиторий, чтобы не изменять историю репозитория, что могло бы создать проблемы у других участников проекта.",
    codeExample: "git revert <commit-hash>",
    skills: ["Git"],
    keywords: ["#revert", "#undo"],
    difficulty: 3,
    rating: 3,
  ...
  },
  {
    ...
    shortAnswer:
      "`git merge` объединяет две ветки: если изменения не пересекаются, создаётся fast-forward (перемещает указатель ветки). При наличии расхождений — создаётся новый merge-коммит, объединяющий родительские ветки, сохраняя обе истории.",
    longAnswer:
  ...
  },
  {
    ...
    longAnswer:
      "`git merge`:\n\n- Fast-forward: если целевая ветка не изменилась, просто двигает указатель.\n- Three-way merge: при расхождении двух веток создаёт новый коммит с двумя родителями.\n- Сохраняет топологию ветвлений и merge-коммиты.\n\n`git rebase`:\n\n- Находит общий предок и «отсоединяет» коммиты текущей ветки.\n- Поочерёдно применяет их на обновлённой базе, создавая новые SHA.\n- Упрощает историю, убирает лишние merge-коммиты.\n\nПлюсы и минусы: merge безопасен, не меняет существующую историю, удобен для публичных веток. Rebase даёт линейную историю, легче читать, но нельзя делать на общих ветках (переписывает историю).\n\nКогда применять: merge — при объединении готовой feature-ветки в main для сохранения контекста. Rebase — при синхронизации feature-ветки с main перед merge или для уборки истории.",
    codeExample:
      "git checkout feature\ngit rebase main   # линейная история\n\ngit checkout main\ngit merge feature  # merge-коммит",
    skills: ["Git"],
  ...
  },
  {
    ...
    shortAnswer:
      "Есть два основных способа: `git cherry-pick` — копирует конкретный коммит в текущую ветку; интерактивный rebase (`git rebase -i`) — позволяет переупорядочивать или переносить несколько коммитов.",
    longAnswer:
      "Использование `git cherry-pick`. Метод подходит, если нужно перенести один или несколько конкретных коммитов.\n\n1. Узнайте хеш коммита: `git log source-branch --oneline`.\n2. Переключитесь на ветку, куда переносите коммит: `git checkout target-branch`.\n3. Выполните cherry-pick: `git cherry-pick abc123`.\n\nGit применит изменения из коммита `abc123` в целевой ветке как новый коммит (с другим хешем). Если возникли конфликты — решите их вручную, затем `git add .` и `git cherry-pick --continue`.\n\nИнтерактивный rebase (`git rebase -i`). Подходит для переноса нескольких коммитов или изменения их порядка. Переключитесь на ветку-источник, запустите `git rebase -i target-branch`, оставьте только нужные коммиты, сохраните файл, затем переключитесь на целевую ветку и выполните слияние.\n\nАльтернатива: `git merge` с выбором коммитов — если нужно перенести все коммиты из одной ветки в другую.\n\nВажные нюансы: cherry-pick создаёт новый коммит (даже если изменения идентичны); rebase переписывает историю — не используйте его для опубликованных веток; если коммит зависит от других изменений, возможны конфликты.",
    codeExample:
      "git checkout target-branch\ngit cherry-pick abc123\n\n# или\ngit rebase -i target-branch",
    skills: ["Git"],
    keywords: ["#cherry-pick", "#rebase"],
    difficulty: 4,
    rating: 4,
  },
  {
    ...
    shortAnswer:
      "`git pull` сразу делает две вещи: загружает изменения с удалённого сервера (`git fetch`) и затем пытается автоматически объединить их с текущей веткой (`git merge`). `git merge` — это только вторая часть процесса: объединение изменений из другой ветки в вашу текущую.",
    longAnswer:
      "Команды `git pull` и `git merge` служат для объединения изменений из разных источников, но делают это на разных уровнях автоматизации.\n\n`git pull` — это комплексная команда, которая выполняет две операции последовательно:\n\n1. `git fetch` — загружает все последние изменения из удалённого репозитория в вашу локальную копию. Эти изменения пока не затрагивают вашу рабочую ветку.\n2. `git merge` — после загрузки изменений `git pull` автоматически пытается объединить (merge) соответствующую удалённую ветку (чаще всего `origin/your-branch-name`) с вашей текущей веткой.\n\n`git merge` — это самостоятельная, более низкоуровневая команда. Она используется для слияния любой ветки с вашей текущей веткой: другой локальной ветки или удалённой ветки, которую вы предварительно загрузили с помощью `git fetch`.\n\nКлючевое отличие: `git pull` = `git fetch` + `git merge`. `git merge` — это только одна из двух операций, которые выполняет `git pull`.\n\nКогда что использовать: `git pull` — чтобы быстро синхронизировать вашу локальную ветку с удалённым репозиторием. `git fetch` + `git merge` отдельно — если вы хотите сначала проверить загруженные изменения, прежде чем объединять их со своей работой. Это считается более безопасным подходом.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#pull", "#merge", "#fetch"],
    difficulty: 3,
    rating: 4,
  },
  {
    ...
    shortAnswer:
      "`git reset` перемещает указатель текущей ветки на конкретный коммит, потенциально удаляя более новые коммиты из истории, и может изменять индекс и рабочую директорию. `git revert` создаёт новый коммит, который отменяет изменения указанного коммита, сохраняя всю историю. Reset изменяет историю и опасен для опубликованных коммитов, revert безопасен для общей истории.",
    longAnswer:
      "`git reset` — перезапись истории. Reset перемещает указатель текущей ветки на указанный коммит.\n\nТри режима reset:\n\n1. `--soft` — только перемещает указатель (`git reset --soft HEAD~1`). История: удаляет последний коммит. Индекс: изменения остаются в staged. Рабочая директория: файлы не изменяются.\n2. `--mixed` (по умолчанию) — перемещает указатель и сбрасывает индекс (`git reset --mixed HEAD~1`). История: удаляет последний коммит. Индекс: изменения становятся unstaged. Рабочая директория: файлы сохраняют изменения.\n3. `--hard` — полное удаление изменений (`git reset --hard HEAD~1`). История: удаляет последний коммит. Индекс: полностью очищается. Рабочая директория: все изменения удаляются.\n\n`git revert` — безопасная отмена изменений. Revert создаёт новый коммит, который отменяет изменения указанного коммита (`git revert HEAD`). История: добавляет новый коммит. Безопасность: не перезаписывает существующую историю. Безопасен для опубликованных коммитов.\n\nКлючевые различия: reset опасен для опубликованной истории, revert безопасен для общей работы; reset удаляет коммиты из истории, revert добавляет новые коммиты в историю. Reset уместен для локальной работы (например, объединить несколько коммитов), revert — когда коммит уже опубликован.",
    codeExample:
      "# Локальная работа\ngit reset --soft HEAD~2\ngit commit -m \"Complete feature implementation\"\n\n# Опубликованный коммит\ngit revert HEAD\ngit push origin master",
    skills: ["Git"],
  ...
  {
    question:
      "Что безопаснее, если стенд отстал от прода и параллельно есть незавершённые задачи — merge или rebase?",
    shortAnswer:
      "В большинстве случаев безопаснее использовать `merge`, особенно если ветка уже используется другими разработчиками. Merge сохраняет историю коммитов и не переписывает её. Rebase переписывает историю и может привести к конфликтам или потере коммитов при неправильном использовании, поэтому его лучше применять только в личных и ещё не опубликованных ветках.",
    longAnswer:
      "Когда стенд или feature-ветка отстаёт от основной ветки, важно выбрать стратегию, которая не сломает историю.\n\nОпределения: merge — это объединение веток с сохранением истории; rebase — это перенос коммитов одной ветки поверх другой с переписыванием истории.\n\nMerge создаёт дополнительный коммит слияния. Плюсы: безопасен для общих веток; история не переписывается; проще откатить изменения. Минусы: история может становиться менее линейной.\n\nRebase «переписывает» коммиты, как будто они были сделаны позже. Плюсы: чистая линейная история; удобно перед merge в main. Минусы: опасен для веток, которые уже запушены; может сломать работу коллег.\n\nПрактическая рекомендация: общие ветки (staging, develop) → merge; личные feature-ветки → rebase перед merge. Когда важна стабильность и над веткой работает несколько человек, безопаснее использовать merge, а rebase оставлять для локальной подготовки кода.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#merge", "#rebase", "#workflow"],
    difficulty: 4,
    rating: 3,
  },
  {
    ...
    longAnswer:
      "Merge — это стандартный и самый безопасный способ объединения веток в Git. Это операция, при которой Git объединяет изменения из одной ветки в другую, сохраняя всю историю коммитов.\n\nКак работает merge: Git ищет общий базовый коммит, применяет изменения обеих веток и создаёт новый merge-коммит.\n\nЧто происходит с историей: она не переписывается, сохраняет все коммиты, становится нелинейной.\n\nMerge-коммит имеет два родителя, фиксирует факт объединения и показывает, какие ветки были слиты.\n\nПреимущества merge: безопасен, не ломает историю, подходит для shared-веток, не требует force push.\n\nНедостатки: история становится сложнее, появляется много merge-коммитов, сложнее читать лог.\n\nMerge сохраняет полную историю разработки и считается самым надёжным способом объединения веток, особенно в командной работе.",
    codeExample: "A---B---C---M\n         \\  /\n          D---E\n\n# M — merge-коммит",
    skills: ["Git"],
    keywords: ["#merge", "#history"],
    difficulty: 3,
    rating: 3,
  },
  {
    ...
    longAnswer:
      "Rebase — это мощный, но требующий осторожности инструмент Git. Это операция, при которой Git переносит коммиты одной ветки поверх другой, переписывая историю.\n\nКак работает rebase: Git находит общий базовый коммит, временно «убирает» коммиты текущей ветки, применяет их поверх новой базы, создаёт новые коммиты с новыми хэшами.\n\nВлияние на историю: история становится линейной, merge-коммиты отсутствуют, каждый коммит имеет новый hash.\n\nПочему это переписывание истории: хотя изменения те же самые, Git создаёт новые коммиты, а старые коммиты больше не существуют в истории. Это и есть причина возможных проблем.\n\nКогда rebase опасен: если ветка уже запущена, если над ней работают другие, если требуется force push.\n\nКогда rebase уместен: локальные feature-ветки, подготовка ветки перед merge, упрощение истории.\n\nRebase делает историю аккуратной и линейной, но требует дисциплины. Его нельзя применять к общим веткам без полного понимания последствий.",
    codeExample:
      "# До:\nA---B---C\n     \\\n      D---E\n\n# После rebase:\nA---B---C---D'---E'",
    skills: ["Git"],
  ...
  },
  {
    ...
    longAnswer:
      "Merge — безопасный, но не всегда удобный инструмент.\n\nОсновные недостатки merge:\n\n- Загромождение истории. При активной разработке появляется много merge-коммитов, история становится визуально сложной, трудно читать `git log`.\n- Шум в истории. Merge-коммит часто не содержит новых изменений, лишь фиксирует факт объединения, усложняет поиск причин багов.\n- Сложность анализа. При анализе сложно понять порядок изменений, труднее делать `git bisect`, больше когнитивной нагрузки.\n\nПочему merge всё равно используют, несмотря на недостатки: merge не переписывает историю, безопасен для командной работы, не требует force push.\n\nГде merge предпочтительнее: `main` / `develop`, shared-ветки, open-source проекты.\n\nMerge ухудшает читаемость истории, но обеспечивает безопасность. Это осознанный компромисс между чистотой истории и надёжностью.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#merge", "#history"],
    difficulty: 3,
  ...
  },
  {
    ...
    longAnswer:
      "Rebase опасен не сам по себе, а при неправильном применении.\n\nОсновной риск — переписывание истории. При rebase: старые коммиты исчезают, создаются новые коммиты, ссылки на старые коммиты ломаются.\n\nПроблемы в командной работе. Если ветка уже запущена: другой разработчик основан на старой истории, rebase меняет базу, возникает конфликт историй, требуется ручное восстановление.\n\nForce push. После rebase часто требуется `git push --force`. Это может затереть чужие коммиты, ломает CI, создаёт хаос.\n\nПотеря изменений. При ошибке: коммиты могут стать недоступны, восстановление требует `git reflog`, не всегда очевидно, что потерялось.\n\nКогда рисков нет: ветка локальная, ветка принадлежит одному разработчику, она ещё не опубликована.\n\nRebase — инструмент для аккуратных рук. Его стоит использовать осознанно и только там, где риск минимален.",
    codeExample: "git push --force   # опасно для общих веток",
    skills: ["Git"],
    keywords: ["#rebase", "#force-push", "#risks"],
    difficulty: 4,
    rating: 4,
  },
  {
    ...
    longAnswer:
      "Процесс релиза в GitFlow построен так, чтобы зафиксировать состояние продукта и минимизировать риски перед выпуском.\n\nЭтапы релиза:\n\n1. Подготовка к релизу. Когда набор фич готов, создаётся ветка релиза: `git checkout develop`, `git checkout -b release/1.2.0`. Ветка фиксирует текущее состояние `develop`, новые фичи сюда уже не добавляются.\n2. Финальные правки. В `release`-ветке выполняются: исправления багов, обновление версии приложения, правки конфигураций и документации.\n3. Слияние в `main`. После тестирования релиз считается готовым: `git checkout main`, `git merge release/1.2.0`, `git tag v1.2.0`. Код становится продакшен-версией, создаётся тег версии.\n4. Возврат изменений в `develop`. Чтобы `develop` не отстал от продакшена: `git checkout develop`, `git merge release/1.2.0`.\n5. Удаление release-ветки. После релиза временная ветка больше не нужна.\n\nGitFlow-релиз — это контролируемый процесс фиксации версии, который позволяет стабилизировать код перед выпуском и сохранить целостность веток разработки.",
    codeExample:
      "git checkout develop\ngit checkout -b release/1.2.0\n# финальные правки\ngit checkout main\ngit merge release/1.2.0\ngit tag v1.2.0\ngit checkout develop\ngit merge release/1.2.0",
    skills: ["Git"],
    keywords: ["#gitflow", "#release"],
    difficulty: 4,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Код-ревью нужно для повышения качества кода и снижения количества ошибок. Оно помогает находить проблемы до попадания кода в продакшен. Также код-ревью выравнивает стиль и подходы внутри команды. Это инструмент обмена знаниями, а не контроля.",
    longAnswer:
      "Код-ревью — это регулярный процесс проверки изменений другими разработчиками перед их объединением в основную ветку.\n\nОсновные задачи код-ревью:\n\n1. Контроль качества: поиск логических ошибок, выявление потенциальных багов.\n2. Поддержка архитектуры: соответствие принятым подходам, предотвращение архитектурного «размывания».\n3. Улучшение читаемости: понятные имена, упрощение сложных участков.\n4. Обмен знаниями: знакомство команды с новым кодом, распространение лучших практик.\n\nЧто код-ревью не должно делать: заменять автотесты, превращаться в субъективную критику, блокировать работу из-за мелочей.\n\nКод-ревью — это механизм коллективной ответственности за кодовую базу и важный элемент масштабируемой разработки. Хорошее код-ревью делает код понятнее и безопаснее.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#code-review", "#workflow"],
    difficulty: 3,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Merge request используется для проверки и обсуждения изменений перед попаданием в основную ветку. Разработчик создаёт ветку, вносит изменения и открывает MR. Код проходит review и автоматические проверки. После одобрения изменения мержатся.",
    longAnswer:
      "Процесс merge request помогает контролировать качество кода и снижать риски при совместной разработке.\n\nОбщая схема процесса:\n\n1. Разработчик создаёт feature-ветку.\n2. Вносит изменения и коммитит код.\n3. Открывает merge request.\n4. Проходят проверки и code review.\n5. Изменения мержатся в целевую ветку.\n\nCode review. Во время review обычно проверяется: корректность логики, читаемость и стиль кода, соответствие архитектурным договорённостям, отсутствие побочных эффектов.\n\nАвтоматические проверки. Перед мержем часто запускаются: линтеры, тесты, сборка проекта. Это позволяет отловить ошибки до попадания в основную ветку.\n\nРоль merge request. MR служит точкой обсуждения изменений, механизмом контроля качества, историей принятых решений.\n\nГрамотно выстроенный процесс merge request повышает стабильность frontend-кода и упрощает командную работу.",
    codeExample:
      "git checkout -b feature/login\n# изменения\ngit push origin feature/login\n# открыть Merge Request",
    skills: ["Git"],
    keywords: ["#merge-request", "#code-review"],
    difficulty: 3,
    rating: 3,
  },
  {
    ...
    longAnswer:
      "Branching-model задаёт структуру работы с репозиторием и влияет на стабильность продукта. Это набор договорённостей о том, какие ветки существуют в репозитории и как они используются.\n\nНаиболее распространённый подход. Во frontend-проектах чаще всего встречается следующая схема:\n\n- `main` или `master` — стабильная production-версия.\n- `develop` — интеграционная ветка.\n- `feature/*` — ветки для разработки отдельных задач.\n- `hotfix/*` — быстрые исправления production.\n\nПроцесс работы:\n\n1. Задача берётся в работу.\n2. Создаётся feature-ветка.\n3. Изменения проходят code review.\n4. Ветка мержится в основную.\n\nПочему это важно. Чёткий branching-model снижает количество конфликтов, упрощает релизы, делает историю изменений понятной, повышает стабильность продукта.\n\nBranching-model — это фундамент командной работы с Git и один из ключевых факторов устойчивого процесса разработки.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#branching-model", "#workflow"],
    difficulty: 3,
    rating: 3,
  },
  {
    question: "Какие типы веток обычно используются в GitFlow?",
    shortAnswer:
      "В GitFlow используются основные ветки `main` и `develop`, а также вспомогательные ветки `feature`, `release` и `hotfix`. Ветка `feature` используется для разработки новых функций, `release` — для подготовки релиза, `hotfix` — для срочных исправлений в production.",
    longAnswer:
      "GitFlow — это модель ветвления, которая разделяет разные этапы разработки.\n\nОсновные ветки GitFlow:\n\n1. `main` — содержит только стабильный, готовый к продакшену код. В `main` всегда находится версия, которую можно выпускать пользователям. Каждый коммит обычно помечается тегом версии. Прямые коммиты в `main` запрещены.\n2. `develop` — основная ветка активной разработки. В неё сливаются все завершённые фичи. Код может быть нестабильным. От неё создаются `feature`, `release` и иногда `hotfix`.\n\nВспомогательные ветки:\n\n3. `feature/*` — используются для разработки новых возможностей. Создаются от `develop`, содержат код одной конкретной задачи, после завершения вливаются обратно в `develop`.\n4. `release/*` — используются для подготовки релиза. Создаются от `develop`, содержат только багфиксы и правки версии, после завершения вливаются в `main` и `develop`.\n5. `hotfix/*` — используются для срочных исправлений в продакшене. Создаются от `main`, после исправления вливаются в `main` и `develop`, позволяют быстро выпускать патчи.\n\nGitFlow чётко разделяет стабильный код, разработку, фичи и экстренные исправления. Он хорошо подходит для проектов с релизным циклом и несколькими разработчиками.",
    codeExample: "git checkout -b feature/login develop",
    skills: ["Git"],
  ...
  },
  {
    ...
    shortAnswer:
      "Code review — это проверка кода другими разработчиками перед объединением в основную ветку. Обычно разработчик создаёт pull request, а коллеги проверяют изменения, ищут ошибки, проблемы архитектуры и несоответствие стандартам. После одобрения код сливается в основную ветку.",
    longAnswer:
      "Code review — это процесс контроля качества кода до его попадания в основной репозиторий.\n\nКак проходит процесс:\n\n1. Разработчик создаёт ветку.\n2. Вносит изменения.\n3. Создаёт Pull Request или Merge Request.\n4. Другие разработчики проверяют код.\n5. Вносятся правки.\n6. Код принимается и мержится.\n\nЧто проверяют на review: читаемость кода, архитектуру, обработку ошибок, безопасность, соответствие style guide.\n\nЗачем нужен code review: снижение количества багов, обмен знаниями в команде, единый стиль кода, контроль архитектуры.\n\nCode review — важный этап разработки, который улучшает качество кода и снижает вероятность ошибок в production.",
    codeExample: undefined,
    skills: ["Git"],
  ...
  },
  {
    ...
    longAnswer:
      "Pre-commit hooks — это автоматические проверки, которые выполняются до фиксации изменений в Git.\n\nКак это работает:\n\n1. Разработчик выполняет `git commit`.\n2. Git запускает hook.\n3. Запускаются проверки.\n4. Если есть ошибки — коммит отменяется.\n\nЧто обычно проверяют: форматирование кода (black), линтеры (flake8), сортировку импортов (isort), статический анализ типов (mypy).\n\nИнструмент pre-commit. Самый распространённый инструмент — библиотека `pre-commit`. Она использует конфигурационный файл `.pre-commit-config.yaml`, где описываются нужные проверки. Установка хуков: `pre-commit install`.\n\nЗачем это нужно: уменьшает количество ошибок, стандартизирует код, автоматизирует проверки.\n\nPre-commit hooks позволяют автоматически проверять код до коммита и поддерживать единое качество кода в команде.",
    codeExample:
  ...
  },
  {
    ...
    longAnswer:
      "Автоматическая проверка кода перед коммитом — это комбинация инструментов и конфигурации.\n\nОсновные шаги настройки:\n\n1. Установить pre-commit: `pip install pre-commit`.\n2. Создать файл `.pre-commit-config.yaml`.\n3. Добавить проверки (репозитории и хуки в конфигурации).\n4. Установить hooks: `pre-commit install`.\n\nКакие проверки обычно добавляют: black, flake8, isort, mypy (для JS-проектов — eslint, prettier, tsc).\n\nДополнительный уровень проверки. Часто дополнительно проверяют код в CI: GitHub Actions, GitLab CI. Это гарантирует, что проверка выполнится даже если hooks отключены.\n\nАвтоматическая проверка перед коммитом снижает количество ошибок и делает код в репозитории более стабильным.",
    codeExample:
      "pip install pre-commit\n\n# .pre-commit-config.yaml\nrepos:\n  - repo: https://github.com/psf/black\n    rev: 23.0\n    hooks:\n      - id: black\n\npre-commit install",
    skills: ["Git"],
    keywords: ["#pre-commit", "#ci-cd", "#linting"],
    difficulty: 4,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Чаще всего используется семантическое версионирование (SemVer), где версия состоит из трёх чисел: major, minor и patch. Major — несовместимые изменения, minor — новая функциональность, patch — исправления ошибок. Версии обычно фиксируются тегами в Git.",
    longAnswer:
      "Версионирование — это способ обозначать состояние проекта и изменения между релизами.\n\nСемантическое версионирование. Формат версии: `MAJOR.MINOR.PATCH`. Значения:\n\n1. MAJOR — несовместимые изменения API.\n2. MINOR — новая функциональность без нарушения совместимости.\n3. PATCH — исправления ошибок.\n\nПример: `2.3.1`.\n\nКак версии используются на практике. Чаще всего: создаётся git tag, формируется release. Пример: `git tag v1.2.0`, затем `git push origin v1.2.0`.\n\nГде используется версионирование: для библиотек, для API, для деплоя.\n\nСемантическое версионирование помогает понимать характер изменений и управлять релизами более предсказуемо.",
    codeExample: "git tag v1.2.0\ngit push origin v1.2.0",
    skills: ["Git"],
    keywords: ["#versioning", "#semver", "#tag"],
    difficulty: 3,
    rating: 3,
  },
  {
    ...
    longAnswer:
      "Commit conventions — это стандартизированный формат сообщений коммитов.\n\nПример Conventional Commits. Формат: `type(scope): описание`. Примеры: `feat(auth): add JWT login`, `fix(api): handle empty response`.\n\nОсновные типы коммитов: `feat` — новая функциональность, `fix` — исправление, `docs` — документация, `refactor` — рефакторинг, `test` — тесты.\n\nЗачем это нужно. Commit conventions позволяют: быстро понимать изменения, автоматически строить changelog, определять тип релиза.\n\nСвязь с CI/CD. Некоторые системы автоматически: увеличивают версию, публикуют релизы.\n\nCommit conventions делают историю изменений структурированной и позволяют автоматизировать релизы и документацию.",
    codeExample: "feat(auth): add JWT login\nfix(api): handle empty response",
    skills: ["Git"],
    keywords: ["#commit-conventions", "#conventional-commits"],
    difficulty: 3,
    rating: 3,
  },
  {
    ...
    longAnswer:
      "Changelog — это список изменений между версиями проекта.\n\nКак происходит генерация. Процесс обычно выглядит так:\n\n1. Разработчики пишут commit-сообщения по стандарту.\n2. Инструмент анализирует историю Git.\n3. Коммиты группируются по типам.\n4. Формируется changelog.\n\nПример commit-сообщений: `feat(auth): add login endpoint`, `fix(api): correct validation`, `docs: update README`. Инструмент распределит изменения по категориям: Features, Bug Fixes, Documentation.\n\nИнструменты. На практике используются: conventional-changelog, semantic-release, git-cliff. Они могут генерировать changelog, повышать версию, создавать релизы.\n\nГде это применяется. Обычно в CI/CD: при создании тега, при сборке релиза.\n\nАвтоматическая генерация changelog снижает ручную работу и делает историю изменений структурированной и понятной.",
    codeExample:
      "feat(auth): add login endpoint\nfix(api): correct validation\ndocs: update README",
    skills: ["Git"],
    keywords: ["#changelog", "#conventional-commits", "#ci-cd"],
    difficulty: 4,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Без линтеров и pre-commit хуков в кодовой базе быстро накапливаются ошибки стиля, потенциальные баги и технический долг. Разные разработчики пишут код по-разному, что ухудшает читаемость и поддержку. Ошибки, которые можно было поймать автоматически, попадают в репозиторий и CI. В итоге увеличивается время code review и растёт риск дефектов в продакшене.",
    longAnswer:
      "Определение. Линтер — это инструмент статического анализа, который автоматически проверяет код на ошибки, нарушения стиля и потенциальные проблемы. Pre-commit хук — это скрипт, который запускается перед коммитом и блокирует его при нарушении заданных правил.\n\nОсновные проблемы при их отсутствии:\n\n1. Нестабильное качество кода: разные стили форматирования, неодинаковые подходы к именованию, сложность чтения и поддержки.\n2. Рост количества простых ошибок: неиспользуемые переменные, опечатки, забытые `await` или `return`.\n3. Перегруженные code review: ревьюеры тратят время на стиль, а не на логику; увеличивается время принятия изменений.\n4. Проблемы в CI/CD: ошибки выявляются слишком поздно, сборки падают из-за того, что можно было поймать локально.\n\nЛинтеры и pre-commit хуки позволяют ловить дешёвые ошибки как можно раньше, уменьшают нагрузку на ревью и стабилизируют качество кода в команде.",
    codeExample: undefined,
    skills: ["Git"],
    keywords: ["#linting", "#pre-commit", "#code-quality"],
    difficulty: 3,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Kanban визуализирует рабочий процесс на доске с колонками (например, «To Do», «In Progress», «Review», «Done»). Каждая задача — карточка, которая перемещается по колонкам. Code review — процесс проверки кода коллегами перед слиянием в основную ветку: карточка переходит в колонку «Review» после завершения разработки, а после успешного ревью — в «Done».",
    longAnswer:
      "Kanban — это гибкая методология управления работой, которая фокусируется на визуализации потока задач и ограничении незавершённой работы (Work In Progress, WIP). Типичные колонки: «Бэклог», «К выполнению» (To Do), «В работе» (In Progress), «Ревью» (Review), «Тестирование», «Готово» (Done). Каждая задача представлена карточкой, которая перемещается слева направо по мере прогресса.\n\nИнтеграция Code Review в Kanban-поток. После того как разработчик завершает работу над задачей, он создаёт pull request (merge request). Карточка задачи переходит в колонку «Review». Ревьюер изучает изменения: корректность логики, соответствие стандартам кодирования, наличие тестов, читаемость. Он оставляет комментарии прямо в интерфейсе pull request. Автор вносит правки, и код снова становится доступным для проверки. После одобрения (approve) ветка сливается в основную (например, `main`), и карточка перемещается в «Готово».\n\nПрактический пример workflow в Git: разработчик создаёт ветку `feature/add-login-button`, коммитит изменения, пушит ветку в удалённый репозиторий и создаёт Pull Request — карточка на доске переходит в «Review».\n\nОграничение WIP в колонке «Ревью» помогает избегать узких мест и ускоряет обратную связь. Совместное использование Kanban и code review создаёт структурированный, прозрачный и итеративный процесс разработки, который минимизирует ошибки и поддерживает стабильный поток выпуска качественного кода.",
    codeExample:
      "git checkout -b feature/add-login-button\ngit commit -m \"Add login button component\"\ngit push origin feature/add-login-button",
    skills: ["Git"],
    keywords: ["#kanban", "#code-review", "#workflow"],
    difficulty: 4,
    rating: 3,
  },
  {
    ...
    shortAnswer:
      "Git Flow — это популярная модель ветвления для Git. Она использует основные ветки: `main` (для продакшена), `develop` (для разработки), а также вспомогательные: `feature` (для новых функций), `release` (для подготовки релиза) и `hotfix` (для срочных исправлений). Эта структура помогает организовать процесс, изолировать изменения и обеспечивать стабильность основной кодовой базы.",
    longAnswer:
      "Git Flow — это модель ветвления, предложенная Винсентом Дриссеном, которая определяет строгие правила создания и слияния веток в Git. Она предназначена для проектов с чётким циклом релизов и помогает управлять параллельной разработкой, стабилизацией и поддержкой.\n\nКлючевые ветки:\n\n- `main` (master): содержит только стабильный, готовый к продакшену код. Каждый коммит здесь — это новый релиз.\n- `develop`: основная ветка для интеграции новых функций. Сюда сливаются завершённые feature-ветки.\n- `feature/*`: создаются от `develop` для разработки новой функциональности. После завершения сливаются обратно в `develop`.\n- `release/*`: создаются от `develop`, когда накоплено достаточно изменений для нового релиза. Здесь идёт финальное тестирование и исправление багов. После готовности сливается в `main` (с тегом версии) и в `develop`.\n- `hotfix/*`: создаётся от `main` для срочного исправления в продакшене. После исправления сливается и в `main`, и в `develop`.\n\nПример рабочего процесса: от `develop` создаётся `feature/user-authentication`, ведётся работа и коммиты, затем ветка сливается обратно в `develop` через `git merge --no-ff` и удаляется.\n\nGit Flow стоит применять в командах, где важна строгая дисциплина релизов, изоляция стадий разработки и поддержка нескольких версий продукта. Для небольших команд и сервисов с частыми деплоями он может быть избыточным — тогда используют более простые модели, такие как GitHub Flow или GitLab Flow.",
    codeExample:
      "git checkout develop\ngit checkout -b feature/user-authentication\ngit commit -m \"Add login form\"\ngit checkout develop\ngit merge --no-ff feature/user-authentication\ngit branch -d feature/user-authentication",
    skills: ["Git"],
    keywords: ["#gitflow", "#workflow"],
    difficulty: 4,
    rating: 4,
  },
  {
    ...
    shortAnswer:
      "Разработка через feature-ветки (Feature Branch Workflow) — это подход, при котором для каждой новой задачи создаётся отдельная ветка от основной (`main` или `develop`). Разработчик работает в изоляции, не затрагивая стабильный код. После завершения работы и тестирования ветка сливается в основную через Pull Request или Merge Request.",
    longAnswer:
      "Разработка через feature-ветки (Feature Branch Workflow) — это подход к управлению версиями, который использует Git для изоляции работы над отдельными задачами. Основная идея в том, что вся разработка новой функциональности ведётся не в основной ветке, а в специально созданной для этой цели ветке.\n\nОсновные шаги процесса:\n\n- Создание ветки: от стабильной основной ветки (`main` или `develop`) создаётся новая ветка. Имя ветки обычно отражает задачу, например `feature/add-user-login` или `fix/header-overflow`.\n- Разработка в изоляции: все коммиты, связанные с этой задачей, делаются внутри созданной feature-ветки. Разработчик может свободно вносить изменения, не беспокоясь о конфликтах с работой других.\n- Интеграция через Pull/Merge Request: после завершения работы ветка не сливается напрямую. Вместо этого создаётся Pull Request (GitHub) или Merge Request (GitLab). Это запускает процесс обсуждения, автоматического тестирования (CI) и обязательного код-ревью.\n- Слияние (Merge): после одобрения PR feature-ветка сливается в целевую ветку. Часто используется стратегия squash merge или rebase, чтобы сохранить историю чистой.\n- Удаление ветки: после успешного слияния feature-ветка обычно удаляется.\n\nЭтот подход является фундаментом для современных практик CI/CD и применяется практически в любой команде, использующей Git. Он особенно полезен для параллельной работы, обязательного код-ревью и интеграции с системами непрерывной интеграции.",
    codeExample:
    ...
    skills: ["Git"],
    keywords: ["#feature-branch", "#workflow"],
    difficulty: 3,
    rating: 4,
  },
  {
    ...
    shortAnswer:
      "Pre-release ветка (часто `release` или `release/*`) — это специальная ветка в Git, куда сливаются все готовые к выпуску фичи и исправления перед финальным релизом. Она служит стабилизационным полигоном для финального тестирования, сборки артефактов и проверки интеграции. Такая ветка изолирует процесс подготовки релиза от активной разработки в `main`/`develop`.",
    longAnswer:
      "Pre-release ветка — это ключевой элемент в стратегиях ветвления, таких как Git Flow. Её основная цель — создать изолированное пространство для подготовки конкретной версии продукта к выпуску, отделяя этот процесс от текущей разработки новых функций.\n\nКак это работает. Когда команда решает, что накопленный в ветке разработки (например, `develop`) функционал готов к выпуску, создаётся новая ветка от `develop`, обычно с именем `release/v1.2.0`. С этого момента:\n\n- В эту ветку вносятся только исправления багов, найденных во время тестирования релиз-кандидата.\n- Разработка новых функций продолжается в `develop` и не попадает в release-ветку.\n- На основе этой ветки запускаются extended-тесты, сборка финальных артефактов (docker-образы, установочные пакеты) и подготовка документации.\n\nПрактическое применение. В CI/CD пайплайне можно настроить автоматическую сборку и развёртывание на staging-окружение именно для веток, начинающихся с `release/`. Это позволяет командам QA и product-менеджерам тестировать именно ту версию, которая скоро выйдет.\n\nТакой подход гарантирует, что тег в `main` соответствует собранному и протестированному артефакту, а все исправления, сделанные во время подготовки релиза, возвращаются в основную ветку разработки (`develop`). Pre-release ветку стоит применять в проектах с регулярными релизами, где важно чёткое разделение этапов разработки, тестирования и поставки.",
    codeExample:
      "git checkout develop\ngit checkout -b release/v1.5.0\ngit commit -m \"fix: resolve issue with login timeout\"\ngit checkout main\ngit merge --no-ff release/v1.5.0\ngit tag -a v1.5.0 -m \"Release version 1.5.0\"",
    skills: ["Git"],
    keywords: ["#release", "#pre-release", "#workflow"],
    difficulty: 5,
    rating: 3,
  },
